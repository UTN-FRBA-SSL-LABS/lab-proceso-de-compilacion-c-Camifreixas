...
> **R:**Porque el preprocesador no compila, sino que expande todo antes de que el compilador vea el código.
...
> **R:** Porque en el archivo no hay rastros de es la palabra pedida
...
> **R:** ntf__,1,0))) __msvcrt_vprintf(const char *, __builtin_va_list);
...
> **R:** El primero no devuelve nada, y el segundo: printf("[DEBUG] %s\n", ("Iniciando main"));
...
> **R:** Vienen del archivo "c:\mingw\include\stdio.h" y comunican lo que viene ahora pertenece a tal archivo en tal línea
...
> **R:**preparan todo para que la función pueda ejecutarse sin romper nada y después volver a la normalidad
...
> **R:**En programa.o, area_circulo es U porque se usa pero no está definida. En matematica.o es T porque ahí sí está implementada.
...
> **R:**quedan símbolos U porque pertenecen a librerías dinámicas y no se incluyen en el ejecutable, son resueltos por el cargador dinámico del sistema en tiempo de ejecución.
...
> **R:**Una macro como CUADRADO(x) es solo texto que se reemplaza y desaparece en el preprocesamiento, sin verificar tipos.
Una función como sumar(a, b) tiene código real, se compila, y sí verifica tipos.
...
> **R:**Un símbolo T es código (funciones) y vive en la sección .text.
Un símbolo D es una variable global inicializada y vive en la sección .data.
